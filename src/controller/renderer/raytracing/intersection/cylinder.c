...
/*   Updated: 2024/01/17 06:55:38 by gdornic          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	height_intersection(t_ray *ray, t_cylinder *cylinder, t_vector *oc)
{
	...
	parameter = intersection_quadratic_minimum(vector_scalar_product(&u, &u), 2 * vector_scalar_product(&v, &u), vector_scalar_product(&v, &v) - powf(cylinder->diameter, 2) / 4);
