...
/*   Updated: 2024/02/11 10:21:57 by gdornic          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_color_init(t_map *map, t_img img)
{
	...
	while (x < map->width)
	{
		...
		while (y < map->height)
		{
			rgb = *(unsigned int *)(img.addr + y * \
			img.line_length + x * (img.bits_per_pixel / 8));
			map->color[x][y] = (t_color){rgb >> 16 & 0xFF, \
			rgb >> 8 & 0xFF, rgb & 0xFF, 0};
			y++;
		}
		x++;
	}
...
void	map_init(t_map *map, char *file_path)
{
	...
	if (instance == NULL)
		return ;
	img.instance = mlx_xpm_file_to_image(instance, file_path, \
